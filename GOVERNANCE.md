...
are real, tested, live, and now live-verified end to end (submission `2026-07-001` merged through
the full pipeline). See `CONTRIBUTING.md`'s own status note. `replay_downscaling.py` and the
private repo's `promote_from_public.py` (the production-promotion side, described further down)
have not been built yet. That is still ahead of us. Today, the maintainer role below is real and
active, and so is the automated referee role. Promotion to production is the piece still
aspirational.
