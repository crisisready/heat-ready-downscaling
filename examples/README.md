...
matters): **fits** a covariate-linear correction (a plain least-squares intercept+slope) on a named
station subset, then **scores** that declared value the same way the referee does
(`score_band`'s `covariate_linear` path never fits anything itself -- it only evaluates an
already-declared value out-of-sample).
...
fraction), fit on all 12 GHCN stations the snapshot carries for that zone. Pass
`--zone`/`--target`/`--covariate` to fit against a different cell or covariate (must be one of
`score.STATIC_COVARIATE_ALLOWLIST` -- see `CONTRIBUTING.md`'s Rung B section for why that list is
what it is).
...
they're run against (`--require-beats-grid`/`--require-pass`, on by default) -- this is what makes
"every example runs in CI" (roadmap Phase 2's own acceptance bar) a real check rather than a
script that merely doesn't crash.
