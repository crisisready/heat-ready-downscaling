...
[`ledger/credit.jsonl`](ledger/credit.jsonl) and [`docs/leaderboard.md`](docs/leaderboard.md).
...
| Rung | What | Status |
|---|---|---|
| **A** | Evaluation coverage — run the validator on a dark cell using the published snapshot | open |
| **B** | Published parameters — bias constants, blend-kernel `L_km`/`R_km`/`tau` | open |
...
Every promotion is independently re-derived against a private copy of the snapshot before
