...
exists in code, and (once the automated scoring harness ships) will earn durable per-cell author
credit with a recorded tenure — visible in [`ledger/credit.jsonl`](ledger/README.md) and
[`docs/leaderboard.md`](docs/leaderboard.md), both empty today (no submissions have gone through
the automated pipeline yet — see "Current status" above), but real, live, and regenerated by
